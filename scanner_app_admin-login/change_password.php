...
$row_admin = $admin_fetch_res->fetch_assoc();
	...
	$user_name_cookie = "username_cookie"; //setting cookie / accessing ccookie value / initializing cookie
	//$_COOKIE  this means get value from cookie
if(isset($_COOKIE[$user_name_cookie]) != '')
{
    $user_name_cookie = $_COOKIE[$user_name_cookie];
	// echo "<script>alert('$user_name_cookie')</script>";
}
...
// // 	window.alert('New Password and Confirm Password doesnot match.')
...
// echo $new_password;
// echo $match_password;
// echo "<script>alert('vendor code _______________')</script>";
// echo "<script>alert('$vendor_code')</script>";
// echo "<script>alert('vendor code _______________')</script>";
// echo "<script>alert('$new_password')</script>";
// echo "<script>alert('$match_password')</script>";

    $get_admin_login_details = $conn->prepare('SELECT * FROM admin_login WHERE admin_username = ?');
	$get_admin_login_details->bind_param('s', $vendor_code);
	$get_admin_login_details->execute();
	$get_admin_login_details_res = $get_admin_login_details->get_result();
	
		$get_admin_login_details_row = $get_admin_login_details_res->fetch_assoc();

	 $ok = $get_admin_login_details_row['admin_password'];
	//  echo "<script>alert('insdide admin login _______________')</script>";
	//  echo "<script>alert('vendor pass')</script>";
	//  echo "<script>alert('$vendor_code')</script>";
	// echo "<script>alert('$ok')</script>";
	// }

	if(password_verify($current_password, $get_admin_login_details_row['admin_password']))
	{
		// echo "<script>alert('allowed updated')</script>";
		if($current_password == $new_password){

			echo "<script>alert('old password and new password cannot be same')</script>";
		}

	else	if ($new_password == $match_password)
     {
         $new_password  = password_hash($new_password, PASSWORD_DEFAULT);
         $update_pass = $conn->prepare('UPDATE admin_login SET admin_password = ? WHERE admin_username = ?');
         $update_pass->bind_param('ss', $new_password, $vendor_code);
        //  $update_pass->execute();
         if($update_pass->execute())
         {
             ?><script>alert('updated the password.');window.location.href = "login.php";</script><?php
         }
         else
         {
     ...
     {
         ?><script>alert('new password and re-entered password did not match. Please try again.');windows.history.back();</script><?php
     }
	}
	else{
...
// 		// $user_name_cookie="test@tester.com";
// 		$new_password=$_POST["new_password"];
...
// 		if($new_password === $c_password){   //check if new and confirm password matches
// 		if ($current_password === $new_password) { //check if old and new password is same
// 			//dont update the password as new-password is same as old password
// 			echo ("<SCRIPT LANGUAGE='JavaScript'>
// 					window.alert('current and new password must not be same. Try Entering New password ' )
				
// 				</SCRIPT>");


// 		}else{  //update the password since the old and new passord is not the same 
// 			//get the old password of the user from database
			
// 			$registered_userData = mysqli_query($conn,"SELECT * FROM `admin_login` WHERE admin_username = '$user_name_cookie'");
...
// 					$sql = "UPDATE admin_login set admin_password ='$hash_password' where admin_username ='$user_name_cookie'  ";
				
// 					echo "<script>alert('inside password verify')</script>";
...
// 						}else{
// 							echo ("<SCRIPT LANGUAGE='JavaScript'>
// 								window.alert('sorry! couldnot complete request please try later.')
...
// 	}
	...
	?>
				...
				<form id="kt_login_update_password_form" method="post" class="form">
					<div class="card-body">
						...
						<div class="form-group row">
							<div class="col-lg-6">
									<label>New Password:</label>
									<input type="text" name="confirm_password" class="form-control" placeholder="Enter New Password" required/>
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-lg-6">
								<button type="submit" id="update_password" name="update_password" class="btn btn-primary mr-2">Update Password details</button>
								<button type="reset" class="btn btn-secondary">Cancel</button>
							</div>
