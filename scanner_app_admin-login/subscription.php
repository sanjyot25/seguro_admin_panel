...
if(isset($_POST['subscription']))
{
	...
	$get_sub_id = $conn->prepare("SELECT id FROM admin_subscription ORDER BY id DESC LIMIT 1");
$get_sub_id->execute();
$get_sub_id_res = $get_sub_id->get_result();
$get_sub_id_row = $get_sub_id_res->fetch_assoc();



$sub_unique_id = $get_sub_id_row['id']; 

$sub_unique_id_second = $sub_unique_id + 1;
$sub_unique_id_third = $sub_unique_id_second + 1;
$sub_unique_id_fourth = $sub_unique_id_second + $sub_unique_id_third;
// Convert a hexadecimal number to octal number:
$sub_unique_id_fourth = base_convert($sub_unique_id_fourth,16,8);
$admin_business_name = 'seguro@';


$sub_id = $admin_business_name."".$sub_unique_id."".$sub_unique_id_fourth;
// echo $sub_id;
    ...
    $sub_dur = $_POST['sub_dur'];
    $devices = $_POST['devices'];

	$user_insert = $conn->prepare('INSERT INTO admin_subscription ( subscription_id,subscription_amount,subscription_no_of_devices,subscription_no_of_days, subscription_name,subscription_details,  subscription_des ) VALUES (?,?,?,?,?,?,?)');
	$user_insert->bind_param('sisssss', $sub_id,$sub_amount,$devices,$sub_dur,$sub_name , $sub_details , $sub_desc );
	$user_insert->execute();

}
	$chapter_fetch = $conn->prepare("SELECT * FROM admin_subscription WHERE status = 1");
	$chapter_fetch->execute();
	$chapter_fetch_res = $chapter_fetch->get_result();
...
<div id="kt_header_mobile" class="header-mobile align-items-center  header-mobile-fixed " >
	<!--begin::Logo-->
	<a href="index.html">
		<img alt="Logo" src="dist/assets/media/logos/logo_mobile.png" class="img-fluid"/>
	</a>
	<!--end::Logo-->
...
<div class="aside aside-left  aside-fixed  d-flex flex-column flex-row-auto"  id="kt_aside">
	...
	<div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper">
		...
		<div
			...
			<!--begin::Menu Nav-->
			<ul class="menu-nav ">
			<li class="menu-item " aria-haspopup="true" ><a  href="view_registered_vendors.php" class="menu-link "><span class="svg-icon menu-icon"><!--begin::Svg Icon | path:dist/dist/assets/media/svg/icons/Design/Layers.svg--><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
		<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
			<polygon points="0 0 24 0 24 24 0 24"/>
	...
	<li class="menu-item " aria-haspopup="true" ><a  href="subscription.php" class="menu-link "><span class="svg-icon menu-icon"><!--begin::Svg Icon | path:dist/dist/assets/media/svg/icons/Design/Layers.svg--><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
	<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        <rect x="0" y="0" width="24" height="24"/>
		...
		<div class="card card-custom gutter-b example example-compact">
			...
			<div class="card-body">
					<div class="form-group row">
						<div class="col-lg-6">
						<span class="form-text text-muted"> enter subsricption name</span>
							<input type="text" name="sub_name" class="form-control" placeholder="subsricption name" required/>
							<div class="valid-feedback">Success! You've done it.</div>
       							<div class="invalid-feedback">No, you missed this one.</div>
							
						</div>
						<div class="col-lg-6">
						<span class="form-text text-muted"> enter subscription details</span>

							<input type="text" name="sub_details" class="form-control" placeholder="subscription details" required/>
							<div class="valid-feedback">Success! You've done it.</div>
       							<div class="invalid-feedback">No, you missed this one.</div>
						</div>
					</div>
					<div class="form-group row">
						<div class="col-lg-6"> 
						<span class="form-text text-muted"> enter subscription amount</span>
							<div class="input-group">
							

								<input type="number" name="sub_amount" class="form-control" placeholder="subscription amount" required/>
								<div class="valid-feedback">Success! You've done it.</div>
       							<div class="invalid-feedback">No, you missed this one.</div>
						...
						<div class="col-lg-6">
						<span class="form-text text-muted"> enter subscription description</span>

							<div class="input-group">
								<input type="text" name="sub_desc" class="form-control" placeholder="subscription description" required/>
								<div class="valid-feedback">Success! You've done it.</div>
       							<div class="invalid-feedback">No, you missed this one.</div>
					...
					<div class="form-group row">
						<div class="col-lg-6">
						<span class="form-text text-muted"> subscription duration in days</span>
							<div class="input-group">
								<input type="number" name="sub_dur" class="form-control" placeholder="subscription duration" required/>
								<div class="valid-feedback">Success! You've done it.</div>
       							<div class="invalid-feedback">No, you missed this one.</div>
						...
						<div class="col-lg-6">
						<span class="form-text text-muted">Please enter no. of devices</span>

							<div class="input-group">
								<input type="number" name="devices" class="form-control" placeholder="add no. of devices" required/>
								<div class="valid-feedback">Success! You've done it.</div>
       							<div class="invalid-feedback">No, you missed this one.</div>
				...
				<div class="card-footer">
					<div class="row">
						<div class="col-lg-6">
						<!-- <button method="post" name="subscription" class="btn btn-primary mr-2">ADD Subscription</button> -->
							
						</div>
						<div class="col-lg-6 text-right">
							<button type="submit" name="subscription" class="btn btn-primary mr-2">ADD Subscription</button>
							<button type="reset" class="btn btn-secondary">Cancel</button>
						</div>
					</div>
				</div>
...
<!--end::Button-->
	...
	<div class="card-body">
		<!--begin: Datatable-->
		<table class="table table-bordered table-checkable" id="subscription">
			                    <thead>
                              <tr>
                                              <th>Sr.NO</th>
					...
					<?php
							...
							if($chapter_fetch_res->num_rows == '0')
							{
								?><tr class="font-weight-boldest border-bottom-0">
								  <td colspan="6" class="border-top-0 pl-0 py-4 text-center"><?php echo "No DATA to be displayed"; ?></td>
							...
							while ($get_orders_row = $chapter_fetch_res->fetch_assoc())
							{
								

								$i++;
							  ?><tr class="font-weight-boldest border-bottom-0">
								  <td class="border-top-0 pl-0 py-4 text-center"><?php echo $i; ?></td>
								  <td class="border-top-0 text-center py-4"><?php echo $get_orders_row['subscription_name']; ?></td>
								  <td class="border-top-0 text-center py-4"><?php echo $get_orders_row['subscription_amount']; ?></td>
								  <td class="border-top-0 pr-0 py-4 text-center"> <?php echo $get_orders_row['subscription_no_of_days']; ?></td>
								  <td class="border-top-0 pr-0 py-4 text-center"> <?php echo $get_orders_row['subscription_details']; ?></td>
								  <td class="border-top-0 pr-0 py-4 text-center"> <?php echo $get_orders_row['subscription_des']; ?></td>
								  <td class="border-top-0 pr-0 py-4 text-center"> <?php echo $get_orders_row['subscription_no_of_devices']; ?></td>
		
								  <td  class="text-center"><a href="subcription/del_subscription.php?row_id=<?php echo $get_orders_row['id'];?>&sub_id=<?php echo $get_orders_row['subscription_id'];?>"  class="btn btn-danger" >delete</a></td>
...
<!--end::Demo Panel-->
	...
	<script>
     ...
     $(document).ready(function(){
		...
		// 	"processing": true,
        // 	"serverSide": true,
        // 	 "ajax": "subcription/server_processing.php",
		// 	 "columnDefs": [
		// 		{
		// 		"data": null,
		...
		// $('#delete_subscription').on( 'click', 'Button', function () {
		// 	var action = this.className;
        // 	// var sub = table.row( $(this).parents('tr') ).data();
		// 	aler('okay');
		// 	var sub_id = document.getElementById('').value;
		...
		// 		// var row_id = $row["id"];
		// 		// var sub_id = $row["subscription_id"];
		// 		alert("hello");
		// 		alert(sub_id);
		// 		$.ajax({
		// 				type: 'POST',
		// 				url: 'subcription/del_subscription.php',
		// 				data: { row_id: row_id, sub_id: sub_id },
