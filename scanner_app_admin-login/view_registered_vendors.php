	...
	$chapter_fetch = $conn->prepare("SELECT * FROM vendor_registration");
	$chapter_fetch->execute();
	$chapter_fetch_res = $chapter_fetch->get_result();
	// $row = $chapter_fetch_res->fetch_assoc();

	// $vendor_code = urlencode($row['vendor_code']);
	...
	<div class="card card-custom gutter-b">
		...
		<h4>All registered vendors at Seguro</h4>
		<!--begin::Dropdown Menu-->
		...
		<div class="card-body">
			...
			<thead>
				<tr>
					...
					<th>Vendor Code </th>
					<th>Vendor Name </th>
					<th>Vendor Bussiness name</th>
					<th>Apply grace period </th>
	...
	<?php
							...
							while($row = $chapter_fetch_res->fetch_assoc()){
								$vendor_code = urlencode($row['vendor_code']);
							...
							<tr>
								...
								<td>'.$row["vendor_full_name"].'</td>
								<td>'.$row["vendor_business_name"].'</td>

								<td><a class="btn btn-primary" href="Add_grace_period.php?vendorcode='.$vendor_code.'">Apply</a></td>
								

								
						
								<td>
								<a class="btn btn-primary" href="view_vendor_details.php?vendor_code='.$vendor_code.'">View</a>
								</td>
								<td>';
								
								if($row['vendor_status']==1){
									echo'	
								<a id="enable" style="pointer-events: none"  class="btn btn-secondary" href="scripts/update_status.php?e_vendorcode='.$vendor_code.'">enable</a>
								</td>';	
								}else{
									echo'	
								<a id="enable" class="btn btn-warning" onClick="alert(\'vendor is enabled!\')" href="scripts/update_status.php?e_vendorcode='.$vendor_code.'">enable</a>
								</td>';
								}
								
								// echo'	
								// <a id="enable" class="btn btn-warning" href="scripts/update_status.php?e_vendorcode='.$row['vendor_code'].'">enable</a>
								// </td>';

								if($row['vendor_status']==2){
									echo '
									<td>
									<a class="btn btn-secondary" style="pointer-events: none" href="scripts/update_status.php?d_vendorcode='.$vendor_code.'">disable</a>
									</td>';
								}else{
									echo '
									<td>
									<a class="btn btn-danger"  onClick="alert(\'vendor is disabled!\')" href="scripts/update_status.php?d_vendorcode='.$vendor_code.'">disable</a>
									</td>';
								}
		...
		<div class="offcanvas-content pr-5 mr-n5">
			...
			<div class="d-flex align-items-center mt-5">
				<div class="symbol symbol-100 mr-5">
					<div class="symbol-label" style="background-image:url('dist/assets/media/users/default.jpg')"></div>
	...
	<!--end::Demo Panel-->
		...
		// 			{
		// 			"data": null,
		// 			"defaultContent": '<button class="btn btn-success">Grace period</button>'
		// 			},
		// 			{
		// 			"data": null,
		// 			"defaultContent": '<button class="btn btn-primary">vendor details</button>'
		// 			},
		// 			{
		// 			"data": null,
		// 			"defaultContent": '<button id="enable" class="btn btn-warning">Enable</button>'
		// 			},
		// 			{
		// 			"data": null,
		// 			"defaultContent": '<button id="disable" class="btn btn-danger">Disable</button>'
		// 			}
		...
		// 	$('#kt_datatable tbody').on( 'click', 'button', function () {
		...
		//     else if(action == 'btn btn-primary'){
		//     // alert( 'This is the Position: '+data[1]);
		//     // var data = table.row( $(this).parents('tr') ).data();
		// 	window.location.href = "view_vendor_details.php?vendor_code="+data[0];
		// 	}
		// 	else if(action == 'btn btn-warning') {

			
		// 		var enable = "enable";
		// 				var record_id = data[0];
		// 				$.ajax({
		// 					type: 'POST',
		// 					url: 'scripts/update_status.php',
		// 					data: { record_id: record_id, enable:enable },
		// 					success: function(response) {
								
		// 						// if(data[3]==1){
		// 						// 		document.querySelector('#enable').disabled = true;
		// 						// }
		// 						alert("updated successfully"+data[0] );
		...
		// 					type: 'POST',
		// 					url: 'scripts/update_status.php',
		// 					data: { record_id: record_id, disable:disable },
		// 					success: function(response) {
		// 						// document.querySelector('#btn').disabled = true;
							
		// 						alert("disabled successfully"+data[1] );
		// 					}
		// 				});
		// 	 }
		// });
